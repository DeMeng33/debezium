...
このリポジトリは Debezium `v1.9.8.Final` をベースにしたカスタム fork であり、
Debezium の公式リリースブランチではありません。

上流の `v1.9.8.Final` tag との主な違いは、Oracle LogMiner に
`plsql_output` という戦略（`log.mining.strategy=plsql_output`）を追加している
点です。この戦略は、`V$LOGMNR_CONTENTS` を JDBC `ResultSet` で直接取得する代わり
に、匿名 PL/SQL ブロックを実行し、`DBMS_OUTPUT` の行を消費して LogMiner の行を
読み取ります。
